...
static const char *typescript_keywords[] = {
    ...
    "var",       "void",     "while",    "with",
    "yield",     "let",      "enum",     "implements",
    "interface", "package",  "private",  "protected",
    "public",    "static",   "await",    "type",
    "namespace", "readonly", "declare",  "as",
    "any",       "unknown",  "never",    NULL};

static int is_typescript_keyword(const char *begin,
                                 const char *end)
{
    size_t len = (size_t)(end - begin);
    for (const char **k = typescript_keywords; *k != NULL;
         ++k)
    {
        if (strlen(*k) == len
    ...
    lumenex_lex_result_t *result)
{
    ...
    while (cursor < end)
    {
        ...
        if (c == '"' || c == '\'')
        {
            ...
            lumenex_token_kind_t tk =
                (delim == '"') ? LUMENEX_TOKEN_STRING
                               : LUMENEX_TOKEN_CHARACTER;
            lumenex_token_t token =
                lumenex_make_token(tk,
                                   start,
                                   literal_end,
                                   start_line,
                                   start_column);
        ...
        if (isalnum(c) || c == '_')
        {
            ...
            lumenex_token_kind_t kind =
                is_typescript_keyword(start, word_end)
